...
namespace MTAIntranetAngular.API
{
    public class ApiResult<T>
    {
        ...
        string? filterQuery = null)
        {
            ...
            {
                // Check if the filter column is the ID column
                if (filterColumn == "id" && int.TryParse(filterQuery, out int idFilter))
                {
                    // Convert ID to string and perform string-based comparison
                    string idFilterString = idFilter.ToString();
                    source = source.Where($"id.ToString().StartsWith(@0)", idFilterString);
                }
                else
                {
                    // Otherwise, use string comparison for other columns
                    source = source.Where(
        ...
        public string? WindowsCurrentUser { get; set; }
        #endregion
    }
}